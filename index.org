...
Enable the runtime you need in =Cargo.toml=:
...
# Or for smol
agent-rex = { version = "0.1", features = ["smol-runtime"] }

# Or for async-std
agent-rex = { version = "0.1", features = ["async-std-runtime"] }

# For library authors: no runtime, users choose their own
agent-rex = { version = "0.1", default-features = false }
#+end_src

*** Core Dependencies

The Rust implementation uses these crates:

#+begin_src toml :tangle no
[dependencies]
futures = "0.3"           # Runtime-agnostic Stream utilities
async-stream = "0.3"      # Convenient stream! macro
...
async-io = { version = "2", optional = true }      # For smol
...
mod tests {
    ...
    async fn test_periodic_emits_at_intervals() {
        ...
        let stream = periodic::<TokioRuntime>(10);
    ...
    #[tokio::test]
    async fn test_periodic_with_timer() {
        let stream = periodic_with_timer(10, |d| tokio::time::sleep(d));
        let values: Vec<_> = stream.take(2).collect().await;
...
mod tests {
    ...
    async fn test_empty_yields_nothing() {
        let stream: futures::stream::Empty<i32> = empty();
    ...
    async fn test_empty_completes_immediately() {
        let stream: futures::stream::Empty<String> = empty();
        futures::pin_mut!(stream);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_never_does_not_emit() {
        let stream: _ = never::<i32>();
        futures::pin_mut!(stream);
        ...
        // Should timeout because never() never yields
...
mod tests {
    ...
    async fn test_scan_product() {
        let source = stream::iter(vec![2, 3, 4]);
...
mod tests {
    ...
    async fn test_filter_even_numbers() {
        let source = stream::iter(vec![1, 2, 3, 4, 5, 6]);
...
mod tests {
    ...
    async fn test_skip_repeats() {
        let source = stream::iter(vec![1, 1, 2, 2, 3, 1, 1]);
    ...
    async fn test_skip_repeats_with_custom_eq() {
        ...
        let source = stream::iter(vec!["apple", "ant", "banana", "berry"]);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_slice() {
        let source = stream::iter(vec![0, 1, 2, 3, 4, 5]);
    ...
    async fn test_slice_empty_range() {
        let source = stream::iter(vec![0, 1, 2, 3, 4]);
    ...
    async fn test_slice_beyond_length() {
        let source = stream::iter(vec![0, 1, 2]);
...
mod tests {
    ...
    async fn test_take_while() {
        let source = stream::iter(vec![1, 2, 3, 4, 2, 1]);
...
/// Uses the Runtime trait for timer functionality.
pub fn debounce<R, T, S>(ms: u64, mut s: S) -> impl Stream<Item = T>
where
    R: Runtime,
    T: Clone,
    S: Stream<Item = T> + Unpin,
{
    stream! {
        ...
        let mut timer: Option<Pin<Box<dyn std::future::Future<Output = ()> + Send>>> = None;
...
pub fn debounce_with<T, S, F, Fut>(
    ms: u64,
    mut s: S,
    sleep_fn: F,
) -> impl Stream<Item = T>
where
    ...
    S: Stream<Item = T> + Unpin,
    F: Fn(Duration) -> Fut + Clone,
    Fut: std::future::Future<Output = ()> + Unpin,
{
    stream! {
        let duration = Duration::from_millis(ms);
        let mut pending: Option<T> = None;
        ...
        loop {
            // Manual select implementation for flexibility
            if let Some(ref mut t) = timer {
                futures::select! {
                    item = s.next() => {
                        match item {
...
mod tests {
    ...
    async fn test_recover_with_error() {
        ...
        let values: Vec<_> = recover_with(
            |_e: Error| stream::iter(vec![99, 100]),
            source,
        ).collect().await;
            ...
            while let Some(item) = s.next().await {
                match item {
                    ...
                    Err(e) => {
                        ...
                        if delay_ms > 0 {
                            R::sleep(Duration::from_millis(delay_ms)).await;
                        }
                        failed = true;
                        break;
                    }
            ...
            while let Some(item) = s.next().await {
                match item {
                    ...
                    Err(e) => {
                        ...
                        if delay_ms > 0 {
                            sleep_fn(Duration::from_millis(delay_ms)).await;
                        }
                        failed = true;
                        break;
                    }
    ...
    async fn test_retry_with_eventual_success() {
        ...
        let values: Vec<_> = retry_with(
            ...
            move || {
                ...
                if n < 2 {
                    stream::iter(vec![Err(Error::new(ErrorKind::Other, "fail"))])
                } else {
                    stream::iter(vec![Ok(42)])
                }
            },
            |_d| std::future::ready(()),
        ).collect().await;
...
The =futures= crate provides built-in merge functionality that is runtime-agnostic:

#+begin_src rust :tangle no
use futures::{Stream, StreamExt, stream};
use async_stream::stream as async_stream;

// Preferred: Use futures::stream::select for two streams (runtime-agnostic)
pub fn merge<T, S1, S2>(s1: S1, s2: S2) -> impl Stream<Item = T>
where
    S1: Stream<Item = T>,
    S2: Stream<Item = T>,
{
...
// For multiple streams, use select_all (runtime-agnostic)
pub fn merge_all<T, S>(streams: Vec<S>) -> impl Stream<Item = T>
where
    S: Stream<Item = T> + Unpin,
{
...
// Custom implementation using futures::select! (runtime-agnostic)
use futures::future::FusedFuture;
use std::pin::Pin;

pub fn merge_custom<T, S1: Stream<Item = T> + Unpin, S2: Stream<Item = T> + Unpin>(
    mut s1: S1,
    mut s2: S2,
) -> impl Stream<Item = T> {
    async_stream! {
        ...
        loop {
            ...
            futures::select! {
                item = s1.next().fuse(), if !s1_done => {
                    match item {
                        Some(v) => yield v,
                        None => s1_done = true,
                    }
                }
                item = s2.next().fuse(), if !s2_done => {
                    match item {
                        Some(v) => yield v,
                        None => s2_done = true,
                    }
...
mod tests {
    ...
    async fn test_merge_all() {
        let streams = vec![
            Box::pin(stream::iter(vec![1, 2])),
            Box::pin(stream::iter(vec![3, 4])),
        ];
    ...
    #[tokio::test]
    async fn test_merge_custom() {
        let s1 = stream::iter(vec![1, 3]);
        let s2 = stream::iter(vec![2, 4]);
        let values: Vec<_> = merge_custom(s1, s2).collect().await;
        assert_eq!(values.len(), 4);
    }
}
#+end_src
...
use std::sync::Arc;
use futures::lock::Mutex;  // Runtime-agnostic
...
mod tests {
    ...
    #[tokio::test]
    async fn test_merge_all_streams() {
        let inner1 = stream::iter(vec![1, 2]);
        let inner2 = stream::iter(vec![3, 4]);
        let outer = stream::iter(vec![inner1, inner2]);
        let values: Vec<_> = merge_all(outer).collect().await;
        assert_eq!(values.len(), 4);
    }
}
#+end_src
...
mod tests {
    ...
    async fn test_flat_map() {
        let source = stream::iter(vec![1, 2]);
        let values: Vec<_> = flat_map(
            |x| stream::iter(vec![x * 10, x * 10 + 1]),
            source,
        ).collect().await;
        assert_eq!(values.len(), 4);
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_switch_map() {
        let outer = stream::iter(vec![1, 2]);
        let values: Vec<_> = switch_map(
            |x| stream::iter(vec![x * 10, x * 10 + 1]),
            outer,
        ).collect().await;
        // Should get values from inner streams
        assert!(!values.is_empty());
    }

    #[tokio::test]
    async fn test_switch_map_empty_outer() {
        let outer = stream::iter(Vec::<i32>::new());
        let values: Vec<_> = switch_map(
            |x| stream::iter(vec![x * 10]),
            outer,
        ).collect().await;
        assert_eq!(values, Vec::<i32>::new());
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_latest2() {
        let s1 = stream::iter(vec![1, 2]);
        let s2 = stream::iter(vec![10, 20]);
        let values: Vec<_> = latest2(s1, s2).collect().await;
        // Emits tuples with latest from each
        assert!(!values.is_empty());
        // All tuples should contain valid combinations
        for (a, b) in &values {
            assert!(*a == 1 || *a == 2);
            assert!(*b == 10 || *b == 20);
        }
    ...
    #[tokio::test]
    async fn test_latest2_one_empty() {
        let s1 = stream::iter(vec![1, 2, 3]);
        let s2 = stream::iter(Vec::<i32>::new());
        let values: Vec<_> = latest2(s1, s2).collect().await;
        // No tuples because s2 never emits
        assert_eq!(values, Vec::<(i32, i32)>::new());
    }
}
#+end_src
...
mod tests {
    ...
    #[tokio::test]
    async fn test_buffer_time_with() {
        // Test with immediate sleep (no real delay)
        let source = stream::iter(vec![1, 2, 3, 4, 5]);
        let values: Vec<_> = buffer_time_with(
            10,
            source,
            |_d| std::future::ready(()),
        ).collect().await;
        // All values should be collected
        let total: Vec<_> = values.into_iter().flatten().collect();
...
#+begin_src rust :tangle no
use std::sync::Arc;
use futures::lock::Mutex;  // Runtime-agnostic async Mutex
use futures::channel::mpsc;
use futures::Stream;
use async_stream::stream;
...
// Alternative: Use async-broadcast crate for true broadcast semantics
// [dependencies]
// async-broadcast = "0.6"
//
...
impl<T: Clone + Send + 'static> Replay<T> {
    ...
    fn subscribe(&self) -> impl futures::Stream<Item = T> {
        ...
        async_stream::stream! {
            let (tx, mut rx) = futures::channel::mpsc::unbounded_channel();
...
The Rust implementation is organized as a library crate with the following module structure:
...
//! For time-based operations, we provide:
//! 
//! - Generic versions that accept a sleep function parameter
//! - Feature-flagged implementations for specific runtimes (tokio, smol, async-std)
//! 
//! ## Example
//! 
//! ```rust
...
pub use concurrent::*;
pub use buffering::*;
pub use multicasting::*;

// Re-export futures for convenience
pub use futures::{Stream, StreamExt};
#+end_src

** =rust/src/runtime.rs= (Runtime Trait)

#+begin_src rust :tangle rust/src/runtime.rs
//! Runtime abstraction for async executor-specific operations.

use std::future::Future;
use std::pin::Pin;
use std::time::Duration;

/// Trait for runtime-specific async operations.
/// 
/// This allows the library to work with different async runtimes
/// (tokio, smol, async-std) by abstracting timer and spawn functionality.
pub trait Runtime {
    /// Sleep for the specified duration.
    fn sleep(duration: Duration) -> Pin<Box<dyn Future<Output = ()> + Send>>;
    
    /// Create an interval that yields at the specified period.
    fn interval(period: Duration) -> Pin<Box<dyn futures::Stream<Item = ()> + Send>>;
...
These are stub modules that can be filled in by extracting the Rust code from each section above.
Each module re-exports the operators defined inline in the documentation.

*** =rust/src/creation.rs=

#+begin_src rust :tangle rust/src/creation.rs
//! Stream creation operators.
//! 
//! Functions for creating streams from various sources.
...
/// Create a stream that emits nothing and completes immediately.
pub fn empty<T>() -> impl Stream<Item = T> {
    futures::stream::empty()
}

/// Create a stream that never emits or completes.
pub fn never<T>() -> impl Stream<Item = T> {
    futures::stream::pending()
}

// Additional creation operators are defined inline in the documentation
// and can be copied here as needed.
#+end_src

*** =rust/src/transformation.rs=

#+begin_src rust :tangle rust/src/transformation.rs
...
pub fn map<T, U, S, F>(f: F, s: S) -> impl Stream<Item = U>
where
    S: Stream<Item = T>,
    F: Fn(T) -> U,
{
...
/// Replace each value with a constant.
pub fn constant<T: Clone, U, S: Stream<Item = U>>(value: T, s: S) -> impl Stream<Item = T> {
    s.map(move |_| value.clone())
}

/// Accumulate values with a reducer function.
pub fn scan<T, U: Clone, S: Stream<Item = T>, F: Fn(U, T) -> U>(
    init: U,
    f: F,
    s: S,
) -> impl Stream<Item = U> {
...
*** =rust/src/filtering.rs=

#+begin_src rust :tangle rust/src/filtering.rs
//! Stream filtering operators.
...
#+begin_src rust :tangle rust/src/slicing.rs
//! Stream slicing operators.
...
/// Take a slice from index start to end (exclusive).
pub fn slice<T, S: Stream<Item = T>>(start: usize, end: usize, s: S) -> impl Stream<Item = T> {
...
#+begin_src rust :tangle rust/src/time.rs
//! Time-based operators.
//! 
//! These operators require a runtime for timer functionality.
//! Use the `_with` variants for runtime-agnostic code.

use futures::{Stream, StreamExt};
use async_stream::stream;
use std::time::Duration;

/// Delay each value by a specified duration (runtime-agnostic version).
pub fn delay_with<T, S, F, Fut>(
    ms: u64,
    s: S,
    sleep_fn: F,
...
*** =rust/src/error_handling.rs=

#+begin_src rust :tangle rust/src/error_handling.rs
...
/// Recover from errors by switching to a fallback stream.
pub fn recover_with<T, E, S, S2, F>(
    recover_fn: F,
    s: S,
) -> impl Stream<Item = T>
...
*** =rust/src/concurrent.rs=

#+begin_src rust :tangle rust/src/concurrent.rs
...
*** =rust/src/buffering.rs=

#+begin_src rust :tangle rust/src/buffering.rs
//! Buffering operators.

use futures::{Stream, StreamExt};
use async_stream::stream;

/// Collect values into fixed-size buffers.
pub fn buffer<T, S: Stream<Item = T>>(size: usize, s: S) -> impl Stream<Item = Vec<T>> {
    stream! {
        futures::pin_mut!(s);
        let mut buf: Vec<T> = Vec::with_capacity(size);
...
*** =rust/src/multicasting.rs=

#+begin_src rust :tangle rust/src/multicasting.rs
//! Multicasting operators for sharing streams among multiple consumers.

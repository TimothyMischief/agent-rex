...
let shared = replay::<TokioRuntime, _, _>(10, source);#+end_src
...
Rust uses =futures::futures::stream::iter= for sync iterables and =async_stream= for async:
...
where
  F: Fn(S) -> UnfoldResult<T, S> + Clone,
{
  futures::stream::unfold(seed, move |state| async move {
    let result = f(state);
    if result.done {
      None
    } else {
...
mod map_tests {
  ...
  async fn test_map_with_type_change() {
    let source = futures::stream::iter(vec![1, 2, 3]);
    let result = map(|x| format!("num-{}", x), source);
    let values: Vec<_> = result.collect().await;
        ...
        item = s.next() => {
          match item {
            ...
            None => {
              ...
              if let Some(value) = pending.take() {  yield value; }
              break;
            }
          }
        }
        _ = async {
          if let Some(ref mut t) = timer {  t.await; }
...
pub fn throttle<T: Clone, S: Stream<Item = T> + Unpin>(
  ...
  stream! {
    ...
    if options.trailing && let Some(value) = trailing_value { yield value; }
  }
}
#+end_src
...
mod recover_with_tests {
  ...
  async fn test_recover_with_no_error() {
    let source = futures::stream::iter(vec![Ok(1), Ok(2), Ok(3)]);
    let values: Vec<_> = recover_with(
      |_e: Error| futures::stream::iter(vec![99]),
      source,
    ).collect().await;
    assert_eq!(values, vec![1, 2, 3]);
  } 
  ...
  async fn test_recover_with_error() {
    ...
    let values: Vec<_> = recover_with(
      |_e: Error| futures::stream::iter(vec![99, 100]),
      source,
    ).collect().await;
  ...
  async fn test_retry_with_eventual_success() {
    ...
    let values: Vec<_> = retry_with(
      ...
      move || {
        ...
        if n < 2 { futures::stream::iter(vec![Err(Error::new(ErrorKind::Other, "fail"))]) }
      ...
      futures::select! {
        item = s1.next().fuse(), if !s1_done => {
          match item {
            Some(v) => {
              latest1 = Some(v);
              if let (Some(ref a), Some(ref b)) = (&latest1, &latest2) { yield (a.clone(), b.clone()); }
            }
            None => s1_done = true,
          }
        }
  ...
  S: futures::Stream<Item = T> + Send + Unpin + 'static,
{
  ...
  async_stream::stream! {
    ...
    futures::pin_mut!(stream);
    while let Some(value) = stream.next().await { yield value; }
  }
}
#+end_src
...
async fn replay_stream_example() {
  let source = futures::futures::stream::iter(vec![1, 2, 3]);
...
cargo doc --all-features --open#+end_src
